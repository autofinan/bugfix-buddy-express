...
export const generateBudgetPDF = async (budget: Budget) => {
  try {
    ...
    budgetItems.forEach((item, index) => {
      ...
      if (item.image_url) {
        try {
          ...
          console.warn('⚠️ Falha ao adicionar imagem do produto');
        }
      }
      ...
      const productName = item.product_name.length > 30 
        ...
        : item.product_name;
      doc.text(productName, margin + 15, y);
    ...
    // Logo GestorMEI no rodapé
    try {
      // Carregar logo do GestorMEI
      ...
      await new Promise((resolve) => {
        gestorReader.onloadend = () => {
          try {
            ...
            console.warn('⚠️ Falha ao adicionar logo GestorMEI no rodapé');
          }
          resolve(null);
        };
    ...
    // Texto "Criado por GestorMEI"
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(100, 100, 100);
    doc.text('Criado por', pageWidth / 2 - 12, footerY + 18);
    
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(59, 130, 246); // Azul GestorMEI
    doc.text('Gestor', pageWidth / 2 + 6, footerY + 18);
    
    doc.setTextColor(16, 185, 129); // Verde GestorMEI
    doc.text('MEI', pageWidth / 2 + 18, footerY + 18);
    ...
    // === SALVAR PDF ===
    const fileName = `Orcamento_${budget.customer_name || 'Cliente'}_${budget.id.substring(0, 6)}.pdf`;
    doc.save(fileName);
