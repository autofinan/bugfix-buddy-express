...
import { ABCCurveFree } from "@/components/reports/ABCCurveFree";
...
export default function ABCCurve() {
  ...
  const loadABCCurve = async () => {
    ...
    setLoading(true);
    try {
      // Chamar função RPC com type assertion
      ...
      if (data && typeof data === 'object' && 'class_a' in data) {
  ...
  // Se for Free, mostra versão free
  if (isFree) {
    return (
