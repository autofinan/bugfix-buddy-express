...
const Index: React.FC = () => {
  ...
  const carregarDashboard = async () => {
    ...
    try {
      ...
      // Vendas recentes - buscar com informações do vendedor
      ...
      if (vendasRecentesError) {
        ...
        const vendasRecentesData = vendasRecentesDireto?.map(venda => {
          const profile = Array.isArray(venda.profiles) ? venda.profiles[0] : venda.profiles;
          return {
            id: venda.id,
            cliente_nome: profile?.full_name || 'Vendedor não identificado',
                  ...
                  vendasRecentes.map(venda => (
                    <tr key={venda.id} className="hover:bg-muted/50">
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm max-w-[120px] sm:max-w-none truncate">
                        {venda.cliente_nome || 'Vendedor não identificado'}
                      </td>
