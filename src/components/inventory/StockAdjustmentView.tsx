...
export default function StockAdjustmentView() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If it's a stock entry AND has purchase value, create automatic expense
      if (adjustmentType === 'in' && purchaseValue && parseFloat(purchaseValue) > 0) {
        const user = (await supabase.auth.getUser()).data.user;
        
        const { error: expenseError } = await supabase
          .from('expenses')
          .insert({
            description: `Compra de estoque: ${selectedProduct.name} (${qty} unidades)`,
            category: 'Compras',
            amount: parseFloat(purchaseValue),
            expense_date: new Date().toISOString().split('T')[0],
            payment_method: 'Dinheiro',
            linked_movement_id: movementData?.id,
            owner_id: user?.id
          });
        ...
        if (expenseError) {
          ...
          toast({
            title: 'Aviso',
            description: 'Movimentação salva, mas erro ao criar despesa automática',
            variant: 'destructive'
          });
        }
      }
      ...
      toast({
        title: "Sucesso",
        description: adjustmentType === 'in' && purchaseValue 
          ? `Estoque atualizado e despesa criada!` 
          : `Estoque de ${selectedProduct.name} atualizado para ${newStock} unidades`
      });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* Purchase Value - Only for stock IN */}
            {adjustmentType === 'in' && (
              <div>
                <label className="text-sm font-medium">Valor Total da Compra (opcional)</label>
                <Input
                  type="number"
                  step="0.01"
                  placeholder="R$ 0,00"
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Ao informar, será criada uma despesa automática na categoria "Compras"
                </p>
              </div>
            )}
