...
export function DashboardStats({ className }: DashboardStatsProps) {
  ...
  const fetchStats = async () => {
    try {
      ...
      const { data: products } = await supabase
        .from('products')
        .select('id, stock, min_stock, cost_unitario, is_active');
      ...
      const stockValue = activeProducts.reduce((acc, p) => 
        acc + (p.stock || 0) * (p.cost_unitario || 0), 0
      );
  ...
  return (
    <div className={`grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 ${className}`}>
